...
# TODO utility functions for scattering ops
def safe_scatter_add_mat(A, ii, jj, n, m):
    v = (ii >= 0) & (jj >= 0) & (ii < n) & (jj < m)
    return scatter_sum(A[:,v], ii[v]*m + jj[v], dim=1, dim_size=n*m)

# TODO difference with the former func
def safe_scatter_add_vec(b, ii, n):
    v = (ii >= 0) & (ii < n)
    return scatter_sum(b[:,v], ii[v], dim=1, dim_size=n)

# apply retraction operator to inv-depth maps using vector addition
def disp_retr(disps, dz, ii):
    ii = ii.to(device=dz.device)
    return disps + scatter_sum(dz, ii, dim=1, dim_size=disps.shape[1])

# apply retraction operator to poses on the SE3 manifold
...
def BA(target, weight, eta, poses, disps, intrinsics, ii, jj, fixedp=1, rig=1):
    ...
    # Step1: commpute jacobians and residuals using projection-based matching
    coords, valid, (Ji, Jj, Jz) = pops.projective_transform(
        poses, disps, intrinsics, ii, jj, jacobian=True)

    r = (target - coords).view(B, N, -1, 1)
    w = .001 * (valid * weight).view(B, N, -1, 1) # Mahalanobis distance

    # Step2: construct linear system
    ...
    # TODO only optimize keyframe poses
    P = P // rig - fixedp
    ii = ii // rig - fixedp
    jj = jj // rig - fixedp
    ...
    # TODO add pixelwise damping factor to the depth block
    C = C + eta.view(*C.shape) + 1e-7

    H = H.view(B, P, P, D, D)
    E = E.view(B, P, M, D, ht*wd)

    # Step3: solve the system in the form of linear equation
    # Inputs: hessian matrix for pose (H), top-right block (E), hessian matrix for depth (C)
    #         value for pose (v) and depth (w) on the right side of normal equation
    # Outputs: delta pose (dx), delta depth (dz)
    dx, dz = schur_solve(H, E, C, v, w)
    
    # Step4: apply retraction to get the final results for pose and depth
...
'''
    Motion-only Bundle Adjustment
        ...
        TODO same as pose optimizer in orbslam? no usage?
'''
